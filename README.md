...
База — расширение RSVData **1.3.0** (релиз автора). В этой ветке — версия **1.3.6**
с доработками:

- инструмент **eventlog** — чтение журнала регистрации 1С с отборами (период,
  уровни, события, приложения, пользователи, поиск по комментарию): область
  `ЖурналРегистрации` в `RSVData_Ядро`, диспетчеризация в `RSVData_Сервер`,
  тема справки `eventlog` (см. `help topic=eventlog`);
- служебная точка **`/update`** — самообновление расширения из кода самого
  расширения (POST тела `.cfe`, с пред-проверкой хеша; платформа 8.3.22+);
- служебная точка **`/remove`** — динамическое самоудаление расширения
  (без монопольного режима);
- `pong` отвечает версией и хешем установленного расширения.

Контракт служебных точек — в [guide.md](guide.md), раздел
«Служебные точки /update и /remove».

## MCP-роутер для нескольких баз

Для работы ИИ-агента с несколькими базами через это расширение есть
**[rsv-data-router](https://github.com/sergeyfedyakov/rsv-data-router)** —
MCP-роутер: одна точка входа в клиенте вместо записи на каждую базу, мапа
«алиас → база», ключи в keyring ОС, установка/обновление/удаление расширения
фоновыми цепочками — как раз через служебные точки `/update` и `/remove`,
без публикации и ручных операций.
...
тег (например `v1.3.6`) на ветке `rsvdata-sources` → заголовок и описание →
